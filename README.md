...
- 📦 **Шаблонные меню** с примерами при первом запуске
...
```yaml
# Название меню (отображается в инвентаре)
title: "&6&lМое меню"

# Размер меню (9, 18, 27, 36, 45, 54)
size: 27

# Скрывать инвентарь игрока при открытии меню
hide_items: false

# Команда для открытия меню (например: /mymenu)
open_command: "mymenu"

# Требования для открытия меню
open_requirements:
  - "permission: example.permission"

# Команды, выполняемые при открытии меню
open_commands:
  - "[console] say Игрок %player% открыл меню!"

# Предметы в меню
items:
  # Идентификатор предмета (любое название)
  example_item:
    # Слот(ы) предмета (один: "13", диапазон: "0-8", список: "0,1,2,3")
    slot: "13"
    ...
    # Отображаемое имя
    display_name: "&b&lПример предмета"
    ...
    # Владелец головы (для PLAYER_HEAD)
    skull_owner: "%player%"
    ...
    # Действия при клике левой кнопкой
    left_click_actions:
      - "[player] me Я кликнул!"
      - "[close]"
    
    # Действия при клике правой кнопкой
    right_click_actions:
      - "[message] &aПравый клик!"
    
    # Требования для клика
    click_requirements:
      - "permission: example.click"
```
